package com.lukitor.projectta.activityPasien

import android.app.AlertDialog
import android.app.ProgressDialog
import android.content.Intent
import androidx.appcompat.app.AppCompatActivity
import android.os.Bundle
import android.os.Handler
import android.view.View
import android.view.WindowManager
import android.widget.Toast
import com.google.firebase.auth.FirebaseAuth
import com.google.firebase.auth.FirebaseUser
import com.google.firebase.database.DataSnapshot
import com.google.firebase.database.DatabaseError
import com.google.firebase.database.FirebaseDatabase
import com.google.firebase.database.ValueEventListener
import com.lukitor.projectta.MainActivity
import com.lukitor.projectta.Model.HistoryPaket
import com.lukitor.projectta.Model.Paket
import com.lukitor.projectta.R
import com.lukitor.projectta.activityAdmin.adapter.MasterPaketAdapter
import com.lukitor.projectta.activityPasien.adapter.ListPaketAdapter
import com.lukitor.projectta.databinding.ActivityListSubsBinding
import com.lukitor.projectta.databinding.ActivityMasterSubsBinding
import java.text.NumberFormat
import java.text.SimpleDateFormat
import java.util.*

class ListSubsActivity : AppCompatActivity() {
    private lateinit var auth: FirebaseAuth
    lateinit var binding: ActivityListSubsBinding
    private lateinit var firebaseUser: FirebaseUser
    private lateinit var listSubs : MutableList<Paket>
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding= ActivityListSubsBinding.inflate(layoutInflater)
        setContentView(binding.root)
        auth = FirebaseAuth.getInstance()
        if (auth.currentUser != null){
            firebaseUser = auth.currentUser!!
            listSubs = mutableListOf()
            Loading()
            loadData()
            binding.imgbackprofilepasien.setOnClickListener {
                finish()
                overridePendingTransition(R.transition.nothing, R.transition.bottom_down)
            }
            binding.rvHistorysaldo.setOnItemClickListener{parent, view, position, id ->
                val paket = listSubs.get(position)
                buyPaket(auth.currentUser!!.uid,paket)

            }

        }
        else{
            signout()
        }
    }

    private fun buyPaket(idpasien:String,paket:Paket){
        val builder = AlertDialog.Builder(this)
        val temp = NumberFormat.getInstance().format(paket.harga)
        var tempstr = "Apakah anda yakin membeli paket " + paket.namaPaket +  "?\nTotal Biaya : Rp. $temp ,-"

        builder.setTitle("Konfirmasi")
        builder.setMessage(tempstr)
        builder.setPositiveButton("Ya") { dialog, which ->
            val calendar = Calendar.getInstance()
            val today = calendar.time
            val daysToAdd = paket.durasi
            calendar.add(Calendar.DAY_OF_YEAR, daysToAdd!!.toInt())
            val futureDate = calendar.time
            val dateFormat = SimpleDateFormat("dd-MM-yyyy", Locale.getDefault())
            val formattedToday = dateFormat.format(today)
            val formattedFutureDate = dateFormat.format(futureDate)
            val ref1 = FirebaseDatabase.getInstance().getReference("HISTORYPAKET")
            val historyId = ref1.push().key
            val namapaket = paket.namaPaket
            val hSaldo = HistoryPaket(historyId.toString(),paket.namaPaket,paket.harga,formattedToday,formattedFutureDate,paket.jmldeteksi,paket.jmlvocer,paket.nominalvocer,idpasien)
            ref1.child(historyId.toString()).setValue(hSaldo).addOnCompleteListener {
                Toast.makeText(this, "Berhasil Membeli Paket $namapaket", Toast.LENGTH_SHORT).show()
            }
        }
        builder.setNegativeButton("Tidak") { dialog, which ->
            Toast.makeText(this, "Pembelian paket ${paket.namaPaket} dibatalkan", Toast.LENGTH_SHORT).show()
        }
        val dialog = builder.create()
        dialog.show()

    }
    private fun loadData(){
        val ref = FirebaseDatabase.getInstance().reference.child("PAKET")
        var ctr = 0
        ref.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if(snapshot.exists()){
                    listSubs.clear()
                    for (h in snapshot.children){
                        val admin = h.getValue(Paket::class.java)
                        if(admin!=null){
                            ctr++
                            listSubs.add(admin)
                            val adapter = ListPaketAdapter(this@ListSubsActivity,R.layout.itemlistsubs,listSubs)
                            binding.rvHistorysaldo.adapter = adapter
                        }
                    }
                    if (listSubs.count()> 0){
                        binding.txtnohistory.visibility = View.GONE
                        binding.rvHistorysaldo.visibility = View.VISIBLE
                    }
                    else{
                        binding.txtnohistory.visibility = View.VISIBLE
                        binding.rvHistorysaldo.visibility = View.GONE
                    }
                }
                else{
                    binding.txtnohistory.visibility = View.VISIBLE
                    binding.rvHistorysaldo.visibility = View.GONE
                }
            }
            override fun onCancelled(error: DatabaseError) {
                TODO("Not yet implemented")
            }
        })
    }

    private fun signout(){
        auth.signOut()
        Intent(this, MainActivity::class.java).also {
            startActivity(it)
            finish()
            overridePendingTransition(R.transition.bottom_up, R.transition.nothing);
        }
    }

    private fun Loading(){
        binding.mainLayout.visibility = View.GONE
        val progressDialog = ProgressDialog(this)
        progressDialog.setMessage("Loading...")
        progressDialog.setCancelable(false)
        progressDialog.show()
        window.setFlags(WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE, WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE)
        Handler().postDelayed({
            progressDialog.dismiss()
            window.clearFlags(WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE)
            binding.mainLayout.visibility = View.VISIBLE
        }, 2000) // 2000 mill
    }
}